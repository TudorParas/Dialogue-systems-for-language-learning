...
def load_hparams(model_dir, verbose=True):
...
def maybe_parse_standard_hparams(hparams, hparams_path, verbose):
...
def save_hparams(out_dir, hparams, verbose):
    """Save hparams."""
...
def format_text(words):
    ...
    if (not hasattr(words, "__len__") and  # for numpy array
        ...
        words = [words]
    return b" ".join(words).decode('utf8')


def format_bpe_text(symbols, delimiter=b"@@"):
    """Convert a sequence of byte-pair encoded words into sentence."""
    words = []
    word = b""
